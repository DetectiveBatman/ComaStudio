  ...
  $.post('/api/getNews', (response) => {
    if (response.ok == 'true'){
      ...
      for (let i = 0; i < response.res.length; i++){
        ...
        let slice = news.enText.slice(0, 400);
        let words = slice.split(' ');
        let preview = news.enText.slice(0, 398 - [words.length - 1].length) + '...';
        let date = news.date;
        let type = news.type;
        ...
        <li class="newsPage-li ${type}">
            ...
            <a class="newsPage-a" href="/En/newsPage?newsId=${id}">
      ...
      document.title = 'News - Coma Studio';
